...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginValues) => {
    ...
    try {
      ...
      const tokens = await api.post<AuthTokens>("/auth/login", formData, {
      ...
      if (res.ok) {
        try {
          const me = await api.get<{ role: string; profile_complete: boolean }>("/auth/me", {
            headers: { Authorization: `Bearer ${tokens.access_token}` },
          });
          
          if (me.role === "candidate" && !me.profile_complete) {
  ...
  return (
    <div className="w-full max-w-[400px] space-y-7">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <a
          href="http://localhost:8000/api/v1/auth/facebook/login"
          className="flex items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50"
        >
